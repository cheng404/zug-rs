use super::Result;
use crate::{Counter, Job, JobStatus, RedisPool, RetryOpts, UnitOfWork, WorkerContext, WorkerRef};
...
impl Chain {
    ...
    #[inline]
    pub(crate) fn iter(&self, context: WorkerContext) -> ChainIter {
        ChainIter {
            stack: self.stack.clone(),
            index: 0,
    ...
    pub(crate) async fn call(
        ...
        let iter = self.iter(context);
        iter.next(job, worker, redis).await?;

        if iter.failed.load(Ordering::SeqCst) {
            Ok(ChainOutcome::Failed)
        } else {
            Ok(ChainOutcome::Success)
        }
    }
}
...
impl ServerMiddleware for HandlerMiddleware {
    ...
    async fn call(
        ...
        worker
            .call_with_context(chain.context(), job.args.clone())
            .await
    }
}
...
impl ServerMiddleware for RetryMiddleware {
    ...
    async fn call(
        ...
        let original_queue = job.queue.clone();

        // Attempt the retry.
        if retry_count > max_retries || job.retry == RetryOpts::Never {
            let mut work = UnitOfWork::from_job(job.clone());
            work.complete(&redis).await?;
            error!({
            ...
            error!({
                "status" = JobStatus::Deferred.as_str(),
                "class" = &job.class,
                "job_id" = &job.job_id,
                "queue" = &job.queue,
            ...
            let mut work = UnitOfWork {
                queue: crate::queue_key(&original_queue),
                job,
            };
            work.requeue(&redis).await?;
        }
...
mod test {
    ...
    fn job() -> Job {
        Job {
            ...
            args: vec![1337].into(),
            retry: RetryOpts::Yes,
            job_id: crate::job::new_job_id(),
            status_ttl_seconds: crate::DEFAULT_JOB_STATUS_TTL_SECONDS,
            created_at: 1337.0,
            enqueued_at: None,
            failed_at: None,
            error_message: None,
