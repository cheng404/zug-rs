...
impl StatsPublisher {
    ...
    pub async fn publish_stats(&self, redis: RedisPool) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let mut command = redis_cmd("HSET");
        command
            .arg(conn.namespaced_key(self.identity.clone()))
            .arg("rss")
            .arg(stats.rss)
            .arg("rtt_us")
            .arg(stats.rtt_us)
        ...
        conn.query_prepared_command::<()>(&mut command).await?;
    ...
    async fn create_process_stats(&self) -> Result<ProcessStats, Box<dyn std::error::Error>> {
        #[cfg(feature = "rss-stats")]
        let rss_in_kb = current_rss_in_kb();
        ...
        Ok(ProcessStats {
            ...
            info: ProcessInfo {
                ...
                started_at: self.started_at.timestamp() as f64,
                pid: std::process::id(),
